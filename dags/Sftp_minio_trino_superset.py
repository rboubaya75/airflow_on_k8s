...
# Define the DAG
with DAG(
    'sftp_to_minio_trino_superset',
    default_args=default_args,
    description='Transfer files from SFTP to Minio, process them, create tables in Trino and add to Superset',
                    ...
                    for root, _, files in os.walk(tmp_dir):
                        for file in files:
                            if file.endswith('.csv'):
                                ...
                                schema_directory = f"parquets/{zip_file.split('/')[-1].replace('.zip', '').replace('-', '_')}"
    ...
    def superset_authenticate():
        ...
        auth_response = session.post(login_endpoint, json=login_payload)
        
        if auth_response.status_code != 200:
            raise Exception(f"Failed to authenticate with Superset: {auth_response.text}")
        ...
        # Get CSRF token
        csrf_response = session.get(csrf_token_endpoint, headers=headers)
        if csrf_response.status_code != 200:
            raise Exception(f"Failed to retrieve CSRF token: {csrf_response.text}")
    ...
    def add_table_to_superset(session, headers, table_name):
        table_payload = {
        ...
        add_table_response = session.post(f"{SUPERSET_URL}/api/v1/dataset/", json=table_payload, headers=headers)
    ...
    def create_table_and_update_superset(**context):
        ...
        # Authenticate with Superset once
        session, headers = superset_authenticate()
        ...
        for schema_dir, parquet_files in schema_directories.items():
            ...
            try:
                ...
                # After creating the table in Trino, add it to Superset
                add_table_to_superset(session, headers, table_name)
            except Exception as e:
